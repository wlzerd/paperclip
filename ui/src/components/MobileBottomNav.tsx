...
export function MobileBottomNav({ visible }: MobileBottomNavProps) {
  ...
  const items = useMemo<MobileNavItem[]>(
    () => [
      { type: "link", to: "/dashboard", label: "홈", icon: House },
      { type: "link", to: "/issues", label: "이슈", icon: CircleDot },
      { type: "action", label: "만들기", icon: SquarePen, onClick: () => openNewIssue() },
      { type: "link", to: "/agents/all", label: "에이전트", icon: Users },
      {
        type: "link",
        to: "/inbox",
        label: "수신함",
